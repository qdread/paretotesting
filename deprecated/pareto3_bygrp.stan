...
parameters {
	...
	vector[M] tau_low_fg;
	real<lower=0,upper=50> sigma_tau_low;
	
	real<lower=x_min, upper=x_max> tau_high_mean;
	vector[M] tau_high_fg;
	real<lower=0,upper=50> sigma_tau_high;
	
	real<lower=0, upper=5> alpha_low_mean;
	vector[M] alpha_low_fg;					// Deviation of each group from mean intercept
	real<lower=0,upper=10> sigma_alpha_low;	// Variation in intercepts
	
	real<lower=0, upper=5> alpha_mid_mean;
	vector[M] alpha_mid_fg;					// Deviation of each group from mean intercept
	real<lower=0,upper=10> sigma_alpha_mid;	// Variation in intercepts
	
	real<lower=0, upper=10> alpha_high_mean;
	vector[M] alpha_high_fg;					// Deviation of each group from mean intercept
	real<lower=0,upper=10> sigma_alpha_high;	// Variation in intercepts
}

model {
	// Prior: three part density
	// No prior set for tau (uniform on its interval)
	alpha_low_mean ~ lognormal(1, 1) T[0, 5];	
	alpha_mid_mean ~ lognormal(1, 1) T[0, 5];	
	alpha_high_mean ~ lognormal(1, 1) T[0, 10];
	...
	for (i in 1:N) {
		x[i] ~ logthreepart(alpha_low_mean + alpha_low_fg[fg[i]], alpha_mid_mean + alpha_mid_fg[fg[i]], alpha_high_mean + alpha_high_fg[fg[i]], tau_low_mean + tau_low_fg[fg[i]], tau_high_mean + tau_high_fg[fg[i]], x_min);
	}
}
...
generated quantities {
	...
	for (i in 1:N) {
		log_lik[i] = logthreepart_lpdf(x[i] | alpha_low_mean + alpha_low_fg[fg[i]], alpha_mid_mean + alpha_mid_fg[fg[i]], alpha_high_mean + alpha_high_fg[fg[i]], tau_low_mean + tau_low_fg[fg[i]], tau_high_mean + tau_high_fg[fg[i]], x_min);
	}
}
