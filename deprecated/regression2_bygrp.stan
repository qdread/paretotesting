...
model {
	...
	x0_fg ~ normal(0, sigma_x0);
	beta0_fg ~ normal(0, sigma_beta0);
	beta1_low_fg ~ normal(0, sigma_beta1_low);
	beta1_high_fg ~ normal(0, sigma_beta1_high);
	...
	// Likelihood: hinged production
	{
	  ...
	  for (i in 1:N) {
		  mu[i] = logistic_hinge(logx[i], x0 + x0_fg[fg[i]], beta0 + beta0_fg[fg[i]], beta1_low + beta1_low_fg[fg[i]], beta1_high + beta1_high_fg[fg[i]], delta);
	  }
...
generated quantities {
	...
	for (i in 1:N) {
		log_lik[i] = normal_lpdf(logy[i] | logistic_hinge(logx[i], x0 + x0_fg[fg[i]], beta0 + beta0_fg[fg[i]], beta1_low + beta1_low_fg[fg[i]], beta1_high + beta1_high_fg[fg[i]], delta), sigma);
	}
}
 */