...
set.seed(7622)
mort_data_sub <- mort_data %>% group_by(fg) %>% sample_n(2000)

# Plot data to remind what it looks like
ggplot(mort_bins, aes(x=bin_midpoint, y=mortality, color=fg)) +
  geom_point() +
  facet_wrap(~ variable, scales = 'free_x') + scale_x_log10() + scale_y_log10()

ggplot(mort_bins %>% filter(variable == "dbh"), aes(x=bin_midpoint, y=mortality, color=fg)) +
  geom_point() +
...
mort_jcurve_fixef_fit <- brm(
  ...
  chains = 4, iter = 2000, warmup = 1000, seed = 27705,
  file = '~/temp/forestlight/mort_jcurve_fixef_brmfit'
)

# mort_jcurve_ranef_fit <- brm(
#   bf(
...
#     nl = TRUE
#   ),
#   data = mort_data_sub, family = bernoulli(link = 'logit'),
#   prior = c(
...
#   ),
#   chains = 4, iter = 2000, warmup = 1000, seed = 27705,
#   file = '~/temp/forestlight/mort_jcurve_ranef_brmfit'
# )
