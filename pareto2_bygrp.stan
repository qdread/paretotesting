...
parameters {
	...
	vector[M] tau_fg;
	real<lower=0,upper=50> sigma_tau;
	...
	real<lower=0, upper=5> alpha_high_mean;
...
model {
	...
	for (i in 1:N) {
		x[i] ~ logtwopart(alpha_low_mean + alpha_low_fg[fg[i]], alpha_high_mean + alpha_high_fg[fg[i]], tau_mean + tau_fg[fg[i]], x_min);
	}
}
...
generated quantities {
	...
	for (i in 1:N) {
		log_lik[i] = logtwopart_lpdf(x[i] | alpha_low_mean + alpha_low_fg[fg[i]], alpha_high_mean + alpha_high_fg[fg[i]], tau_mean + tau_fg[fg[i]], x_min);
	}
}
