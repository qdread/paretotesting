...
model {
	...
	x0_fg ~ normal(mu_x0, sigma_x0);
	beta0_fg ~ normal(mu_beta0, sigma_beta0);
	beta1_low_fg ~ normal(mu_beta1_low, sigma_beta1_low);
	beta1_high_fg ~ normal(mu_beta1_high, sigma_beta1_high);
	...
	// Likelihood: hinged production
	{
	  ...
	  for (i in 1:N) {
		  mu[i] = logistic_hinge(logx[i], x0_fg[fg[i]], beta0_fg[fg[i]], beta1_low_fg[fg[i]], beta1_high_fg[fg[i]], delta);
	  }
...
generated quantities {
	...
	for (i in 1:N) {
		log_lik[i] = normal_lpdf(logy[i] | logistic_hinge(logx[i], x0_fg[fg[i]], beta0_fg[fg[i]], beta1_low_fg[fg[i]], beta1_high_fg[fg[i]], delta), sigma);
	}
}
 */